...
```
export OPENAI_API_KEY=sk-cvexxxxxxxxgx24A5HS4dKn04HU2L
```
...
prompt下的指令很重要，所以現在多了一種學問叫做提示工程學(prompt engineering)



> shellgpt.py: 使用多線程(thread)，呼叫chatGPT，把寫完的文字檔放入md檔。這邊使用slot filling，讓我們使用模板，並改裡面的關鍵字就可以。
...
def handleCommand(command):   # shell 指令
    ...
    if op == 'quit':
        ...
        PSTACK = []
        printStack()
    elif op == 'ppush':  # 推入到 PSTACK 提示堆疊，讓產生出來的內容會比較相關
